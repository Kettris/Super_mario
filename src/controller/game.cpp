...
void Game::check_vertically_static_collisions() noexcept {
    if (!static_objs.empty()) {
        Rect* last_obj = static_objs.back();
        if (mario->has_collision(last_obj)) {
            is_level_end_ = true;
            return;
        }
    }
    ...
    for (Rect* static_obj: static_objs) {
        if (mario->has_collision(static_obj)) {
            mario->process_vertical_static_collision(static_obj);
            break;
        }
    }
